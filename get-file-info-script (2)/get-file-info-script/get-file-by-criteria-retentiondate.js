...
let dateStart = format.parse(format.ISO8601_FORMAT,config.returnToLongTerm);
 // Your timezone!
let returnToLongTermDate = dateStart.getTime();
console.log(returnToLongTermDate);
...
const csvWriter = createCsvWriter({
  ...
  header: [
	...
	{id: 'returnToLongTermFile', title: 'ReturnToLongTerm'},
        ...
        db.query(fileMetadataParams, async function (err, result) {
            if (err) {
                ...
                if (result.Count > 0 && "Metadata" in result.Items[0]) {
                    await printLogs(result, item.PublicId,item.Id,item.CreationDate,item.ArchivedDate,item.DeletedAt,item.State,item.ReturnToLongTerm);
                }
                return resolve();
            }
        });
...
async function printLogs(result, publicId,id,creationDate,archivedDate,deletedAt,state,returnToLongTerm) {
    return new Promise((resolve) => {
		...
		var returnToLongTermNormalFormat = new Date(parseInt(returnToLongTerm));
		const data = [{businessNo:config.bu, publicId:publicId,fileName: result.Items[0].Metadata.FileName,privateId:id,
		creationDateFile:creationDateNormalFormat,archievedDateFile:archivedDateNormalFormat,deletedAtFile:deletedAtNormalFormat,
		stateFile:state,returnToLongTermFile:returnToLongTermNormalFormat}];
